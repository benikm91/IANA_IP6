...
public:
    ...
    DrivingForwardAction(double distance, double speed = DRIVING_SPEED) : m_distance(distance), m_drivingSpeed(speed) { }
    ...
    void Update(std::shared_ptr<VelocityChanger> velocityChanger, Quaternion orientation, Vector3 position)
    {
	    ...
	    ROS_INFO_STREAM("DrivingForwardAction - Current " << (m_startPosition - position).Length() << " Goal: " << m_startPosition);
